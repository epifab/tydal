...
class PostgresQueryBuilderTest extends FlatSpec {
  ...
  val studentId = Field("id", students)
  val studentName = Field("name", students)
  val studentEmail = Field("email", students)

  val exams: DataSource = Table("exams", "e")
  val examRate: Field = Field("rate", exams)
  val examStudentId: Field = Field("student_id", exams)
